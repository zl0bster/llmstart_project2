...
- `/` - простой health check для Railway
- `/debug` - отладочная информация о переменных окружения
- `/health/simple` - проверка БД с таймаутом
- `/health` - полная диагностика компонентов

### 9. Рекомендуемая конфигурация для продакшна
