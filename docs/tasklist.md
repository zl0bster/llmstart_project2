...
**Текущий статус**: Проект в начальной стадии  
**Выполнено итераций**: 0/10  
**Последнее обновление**: 2025-01-27  

### История итераций
| № | Итерация | Статус | Результат | Дата |
|---|----------|--------|-----------|------|
| 1 | Базовый бот и конфигурация | ⏳ Планируется | Бот отвечает на /start и эхо-сообщения | - |
| 2 | Текстовый пайплайн (MVP LLM) | ⏳ Планируется | Извлечение заказов/статусов из текста | - |
| 3 | Хранение данных и сессии | ⏳ Планируется | Сохранение подтвержденных проверок | - |
| 4 | Голосовой пайплайн | ⏳ Планируется | Голос → текст → подтверждение → БД | - |
...
### Задачи:
- [ ] Создать полную структуру проекта согласно @vision.md (строки 59-121)
- [ ] Создать все необходимые директории (app/, cache/, data/, logs/, prompts/, tests/, docker/)
- [ ] Создать пустые __init__.py файлы в пакетах
- [ ] Настроить requirements.txt с базовыми зависимостями
- [ ] Создать .env.example с переменными окружения согласно @vision.md (строки 554-683)
- [ ] Реализовать app/core/config.py (Pydantic BaseSettings) с полной схемой конфигурации
- [ ] Создать app/main.py - точку входа aiogram приложения
- [ ] Реализовать базовые обработчики (/start, эхо-сообщения)
- [ ] Настроить базовое логирование (файлы + консоль)
- [ ] Создать README.md с инструкциями по запуску
- [ ] Протестировать: бот запускается и отвечает на команды

### Документация:
- [ ] Обновить README.md с Quick Start
- [ ] Создать .env.example с комментариями и всеми переменными из @vision.md
...
### Задачи:
- [ ] Добавить зависимости для LLM (openai, pydantic, langchain)
- [ ] Создать app/clients/llm_client.py - клиент для OpenRouter API
- [ ] Создать app/models/schemas.py - Pydantic модели (OrderData, LLMResponse)
- [ ] Создать prompts/system_prompts.py - системные промпты
- [ ] Реализовать app/services/session_manager.py - управление сессиями
- [ ] Обновить app/bot/handlers/text_handlers.py - обработка через LLM
- [ ] Добавить app/bot/keyboards.py - кнопки подтверждения
- [ ] Реализовать цикл подтверждения (Согласен/Исправить/Отменить)
- [ ] Добавить логирование LLM запросов/ответов
- [ ] Создать тестовые сценарии на основе @scen1_use_cases.md
- [ ] Протестировать извлечение данных из примеров прямого текста
- [ ] Протестировать извлечение данных из примеров аудио-транскрипции
- [ ] Протестировать извлечение данных из примеров OCR текста
- [ ] Протестировать: текст → извлечение данных → подтверждение
...
### Задачи:
- [ ] Добавить SQLAlchemy в requirements.txt
- [ ] Создать app/models/database.py - SQLAlchemy модели (User, Inspection, Dialogue)
- [ ] Создать app/core/database.py - подключение к SQLite
- [ ] Реализовать миграции БД (создание таблиц)
- [ ] Обновить Session Manager для работы с БД
- [ ] Создать app/services/data_service.py - бизнес-логика сохранения
- [ ] Реализовать состояния системы согласно @vision.md (строки 123-165)
- [ ] Создать State Machine для переходов между состояниями (idle, processing, clarification, confirmation, cancellation, reports_menu, report_processing)
- [ ] Реализовать UI элементы для каждого состояния (клавиатуры, сообщения)
- [ ] Добавить обработку переходов между состояниями
- [ ] Реализовать сохранение только после подтверждения пользователя
- [ ] Добавить логирование операций с БД
- [ ] Создать команду /status для проверки данных
- [ ] Протестировать: подтверждение → сохранение в БД → проверка
- [ ] Протестировать переходы между состояниями согласно @scen1_user_flow.md

### Документация:
- [ ] Обновить README.md с описанием БД
...
### Задачи:
- [ ] Добавить зависимости для обработки аудио
- [ ] Создать app/clients/speech_client.py - клиент для Whisper API
- [ ] Создать app/services/media_processor.py - обработка медиа файлов
- [ ] Реализовать app/bot/handlers/voice_handlers.py
- [ ] Настроить сохранение аудио в cache/audio/
- [ ] Интегрировать транскрипцию в текстовый пайплайн
- [ ] Добавить валидацию аудио файлов (размер, длительность)
- [ ] Реализовать логирование обработки голоса
- [ ] Добавить обработку ошибок Whisper API
- [ ] Создать тестовые аудио файлы на основе примеров из @scen1_use_cases.md
- [ ] Протестировать транскрипцию примеров голосовых сообщений
- [ ] Сравнить результат транскрипции с эталонными текстами
- [ ] Протестировать: голос → транскрипция → анализ → сохранение

### Документация:
- [ ] Обновить README.md с требованиями к аудио
...
### Задачи:
- [ ] Создать app/clients/vision_client.py - клиент для GPT-4 Vision
- [ ] Реализовать app/bot/handlers/photo_handlers.py
- [ ] Настроить сохранение фото в cache/photos/
- [ ] Добавить валидацию изображений (размер, формат, разрешение)
- [ ] Интегрировать анализ изображений в текстовый пайплайн
- [ ] Реализовать логирование Vision API запросов
- [ ] Добавить обработку ошибок Vision API
- [ ] Создать специализированные промпты для анализа протоколов
- [ ] Добавить поддержку альтернативных Vision моделей
- [ ] Создать тестовые изображения протоколов с текстом из @scen1_use_cases.md
- [ ] Протестировать OCR извлечение текста с изображений
- [ ] Сравнить результат OCR с эталонными текстами
- [ ] Протестировать анализ сложных протоколов (множественные заказы)
- [ ] Протестировать: фото → анализ → извлечение данных → сохранение

### Документация:
- [ ] Обновить README.md с требованиями к изображениям
...
**Цель**: Реализовать генерацию отчетов для администрации  
**Критерий готовности**: Админ получает сводку и CSV за выбранный период  
...
- [ ] Добавить роли пользователей (inspector, admin, manager)
- [ ] Реализовать проверку прав доступа к отчетам
